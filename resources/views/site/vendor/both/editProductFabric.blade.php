        ...
        <nav class="navbar navbar-expand-lg navbar-dark bg-transparent container pt-2">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav mr-auto">
                    <li class="nav-item">
                        <a class="nav-link MontserratArabicLightPure" href="{{route('vendor.aboutBoth')}}"
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="{{route('vendor.getSearchPageBoth')}}"
                        ><i class="fa fa-search" aria-hidden="true"></i>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link MontserratArabicLightPure" href="{{route('vendor.contactUsBoth')}}">اتصل بنا</a>
                    </li>
                    <li class="nav-item dropdown">
                        <a
                            class="nav-link dropdown-toggle"
    ...
    <div class="container my-5">
        <div class="row ">
            <div class="col text-right d-flex p-0">
                <h4  class="pr-2 border-3 section-title" >تعديل منتج - {{$fabric->name}}</h4>
            </div>
        </div>
        <div class="row my-5">
            <div class="col-md-6 col-sm-12">
                <form action="{{route('vendor.updateProductFabric',$fabric->id)}}" method="POST" class="needs-validation pr-1" novalidate enctype="multipart/form-data">
                    @csrf

                    <input type="hidden" name="vendor_id" value="{{$fabric->product->vendor_id}}">
                    ...
                    <div class="form-row my-2">
                        ...
                        <input type="text" name="name" value="{{$fabric->name}}" class="form-control" id="validationCustom01">
                    ...
                    <div class="form-row my-2">
                        <div class="col-md-6 p-0">
                            ...
                            <select name="color_id" id="exampleFormControlSelect1" class="form-control">
                                <optgroup label="الرجاء اختر نوع الملابس">
                                    @isset($data['colors'])
                                        @foreach($data['colors'] as $color)
                                            <option value="{{$color->id}}" @if($fabric_colors->contains('id', $color->id) ==   $color->id) selected @endif>{{$color->color}}</option>
                                        @endforeach
                                    @endisset
                                </optgroup>
                            </select>
                    ...
                    <div class="form-row my-2">
                        ...
                        <textarea name="description" class="form-control" id="validationCustom01" rows="5">{{$fabric->description}}</textarea>
                        @error('description')
...
{{--                            <input id="file-upload" type="file" name="images[]" multiple>--}}
                        ...
                        <div class="col-md-6 pl-0 d-flex">
                                @foreach($fabric->images as $image)
                                    <img src="{{$image->getPhotoFabric($image->photo)}}" style="width: 150px;height: 150px" alt="photo"
                    ...
                    <div class="form-row my-2">
                        <div class="col-md-6 p-0">
                            ...
                            <input type="number" step="any" name="price" value="{{$fabric->product->price}}" class="form-control" id="validationCustom01">
                            @error('price')
                        ...
                        <div class="col-md-6 pl-0">
                            ...
                            <input type="text" name="offer" value="{{$fabric->product->offer}}" class="form-control" id="offer">
                            @error('offer')
                            ...
                            <div class="input-group ">
                                ...
                                <div class="input-group-prepend">
                                    <button type="submit" class=" rounded-left border-right-0 bg-dark text-white input-group-text" >
                                        <img src="{{asset('assets/front/assets/send.png')}}" alt="">
                                    </button>
                                </div>
                            </div>

...
@section('content')
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <a class="btn btn-outline-success float-left" href="javascript:void(0)"
                                       id="addNewHeaderBottomIndex"><i class="la la-plus"></i> اضافة رأسية
                                        جديدة</a>
                                ...
                                <div class="card-content collapse show" id="viewAboutUs">
                                    <div class="card-body card-dashboard table-responsive">
                                        <table class="table header-bottom-index-table">
                                            <thead>
                                            <tr>
                                                <th>النوع</th>
                                                <th>الوصف</th>
    ...
    <div class="modal fade modal-open" id="headerBottomIndex-modal" aria-hidden="true">
            ...
            <div class="modal-content width-600">
                ...
                <div class="modal-body">
                    <div class="card-content collapse show">
                        <div class="card-body">
                            <form class="form" id="headerBottomIndexForm" enctype="multipart/form-data">
                                @csrf
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea name="description" id="description" cols="3" rows="6"
                                                          class="form-control"
                                                          placeholder="نحن شركة زياد مشتهى للاقمشة ..."></textarea>
                                ...
                                <div class="form-actions">
                                    ...
                                    <button class="btn btn-primary" id="addHeaderBottomIndex"><i
                                            class="la la-save"></i> حفظ
                                    </button>
                                </div>
...
@section('script')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            //Show Table
            var headerBottomIndexTable = $('.header-bottom-index-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{route("index.header_bottom")}}",
                columns: [
                    {data: 'type', name: 'type'},
            ...
            //Show Form
            $('#addNewHeaderBottomIndex').click(function () {
                $('#headerBottomIndexForm').trigger('reset');
                $('#headerBottomIndex-modal').modal('show');

            });


            //Add Or Update
            $(document).on('click', '#addHeaderBottomIndex', function (e) {
                e.preventDefault();
                var formData = new FormData($('#headerBottomIndexForm')[0]);
                $('#description_error').text('');
                $('#type_error').text('');
                $('#photo_error').text('');
                $.ajax({
                    type: 'post',
                    url: "{{ route('save.header_bottom') }}",
                    ...
                    success: function (data) {
                        if (data.status == true) {
                            ...
                            $('#headerBottomIndexForm').trigger('reset');
                            $('#headerBottomIndex-modal').modal('hide');
                            headerBottomIndexTable.draw();
                        } else {
            ...
            $('body').on('click', '.deleteHeaderBottomIndex', function () {
                var id = $(this).data('id');

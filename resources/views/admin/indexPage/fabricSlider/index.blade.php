...
@section('content')
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <a class="btn btn-outline-success float-left" href="javascript:void(0)"
                                       id="addNewFabricSliderIndex"><i class="la la-plus"></i>  اضافة سلايدر قماش جديد</a>
                                ...
                                <div class="card-content collapse show" id="viewAboutUs">
                                    <div class="card-body card-dashboard table-responsive">
                                        <table class="table fabric-slider-index-table">
                                            <thead>
                                            <tr>
                                                <th>العنوان</th>
                                                <th>الوصف</th>
    ...
    <div class="modal fade modal-open" id="fabricSliderIndex-modal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content width-600">
                <div class="modal-header">
                    <h4 class="modal-title form-section" id="modalheader">
                        <i class="ft-home"></i> اضافة رأسية قماش
                    </h4>
                ...
                <div class="modal-body">
                    <div class="card-content collapse show">
                        <div class="card-body">
                            <form class="form" id="fabricSliderIndexForm" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="form-actions">
                                    ...
                                    <button class="btn btn-primary" id="addFabricSliderIndex"><i class="la la-save"></i> حفظ</button>
                                </div>
...
@section('script')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            //Show Table
            var FabricSliderIndexTable = $('.fabric-slider-index-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{route("index.fabric_slider")}}",
                columns: [
            ...
            //Show Form
            $('#addNewFabricSliderIndex').click(function () {
                $('#fabricSliderIndexForm').trigger('reset');
                $('#fabricSliderIndex-modal').modal('show');

            });


            //Add Or Update
            $(document).on('click', '#addFabricSliderIndex', function (e) {
                e.preventDefault();
                var formData = new FormData($('#fabricSliderIndexForm')[0]);
                $('#header_error').text('');
                $('#description_error').text('');
                $('#photo_error').text('');
                $.ajax({
                    type: 'post',
                    url: "{{ route('save.fabric_slider') }}",
                    ...
                    success: function (data) {
                        if (data.status == true) {
                            ...
                            $('#fabricSliderIndexForm').trigger('reset');
                            $('#fabricSliderIndex-modal').modal('hide');
                            FabricSliderIndexTable.draw();
                        } else {
            ...
            $('body').on('click', '.deleteFabricSliderIndex', function () {
                var id = $(this).data('id');

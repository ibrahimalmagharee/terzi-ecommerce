        ...
        <div class="content-wrapper">
            <div class="content-header row">
                <div class="content-header-left col-md-6 col-12 mb-2">
                    <h3 class="content-header-title"> الأقمشة </h3>
                    <div class="row breadcrumbs-top">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">الرئيسية </a></li>
                                <li class="breadcrumb-item active"> الأقمشة</li>
                            </ol>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <a class="btn btn-outline-success float-left" href="javascript:void(0)"
                                       id="addNewFabric"><i class="la la-plus"></i>  اضافة قماش جديد</a>
                                ...
                                <div class="card-content collapse show" id="viewFabric">
                                    <div class="card-body card-dashboard table-responsive">
                                        <table class="table fabric-table">
                                            <thead>
                                            <tr>
                                                <th>التاجر</th>
                                                <th>الاسم</th>
...
{{--                                            <div class="col-md-3">--}}
{{--                                                <select name="colors[]" data-colors="2" id="colors_filter" class="select2 form-control" multiple>--}}
{{--                                                    <optgroup label="الرجاء اختر اللون المناسب">--}}
{{--                                                        @isset($data['colors'])--}}
...
{{--                                            </div>--}}
    ...
    <div class="modal fade modal-open" id="fabric-modal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content width-800">
                <div class="modal-header">
                    <h4 class="modal-title form-section" id="modalheader">
                        <i class="ft-home"></i> اضافة قماش
                    </h4>
                ...
                <div class="modal-body">
                    <div class="card-content collapse show">
                        <div class="card-body">
                            <form class="form" id="fabricForm" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="form-body">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select name="colors[]" id="color_id" class="select2 form-control" style="width: 350px" multiple>
                                                    <optgroup label="الرجاء اختر اللون المناسب">
                                                        @isset($data['colors'])
                                                            @foreach($data['colors'] as $color)
                                                                <option value="{{$color->id}}">{{$color->color}}</option>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="projectinput1"> السعر  <span class="text-danger">( بالمتر )</span></label>
                                ...
                                <div class="form-actions">
                                    ...
                                    <button class="btn btn-primary" id="addFabric"><i class="la la-save"></i> حفظ</button>
                                </div>
            ...
            <div class="modal-content">
                ...
                <form id="delete_modal_form">
                    ...
                    <div class="modal-body">
                        ...
                        <h5>هل أنت متأكد من حذف هذا القماش !!</h5>
                    </div>
...
@section('script')
    ...
    //Show Table
            var fabricTable = $('.fabric-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{route("index.fabrics")}}",
                columns: [
            ...
            $('select[name="vendor"]').on('change',function(){
                fabricTable.column($(this).data('vendor')).search($(this).val()).draw();

            });

            $('#name_filter').on('keyup',function(){
                fabricTable.column($(this).data('name')).search($(this).val()).draw();

            });

            $('select[name="colors"]').on('change',function(){
                fabricTable.column($(this).data('colors')).search($(this).val()).draw();

            });

            $('select[name="category_id"]').on('change',function(){
                fabricTable.column($(this).data('category_id')).search($(this).val()).draw();

            });

            $('#price_filter').on('keyup',function(){
                fabricTable.column($(this).data('price')).search($(this).val()).draw();

            });

            $('#offer_filter').on('keyup',function(){
                fabricTable.column($(this).data('offer')).search($(this).val()).draw();

            });

            $('select[name="status"]').on('change',function(){
                fabricTable.column($(this).data('status')).search($(this).val()).draw();

            });

            //Show Form
            $('#addNewFabric').click(function () {
                $('#fabricForm').trigger('reset');
                $('#fabric-modal').modal('show');

            });


            //Add Or Update
            $(document).on('click', '#addFabric', function (e) {
                e.preventDefault();
                var formData = new FormData($('#fabricForm')[0]);
                    ...
                    success: function (data) {
                        if (data.status == true){
                            ...
                            if (data.product_status == 1) {
                                if (data.product_type === 'App\\Models\\Fabric') {
        ...
        Dropzone.options.dpzMultipleFiles = {
            ...
            ,
            url: "{{route('save.images.fabric.inFolder')}}", // Set the url
            success:
            ...
            removedfile: function(file)
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{route('delete.image.fabric.fromFolder')}}',
                    data: {filename:name},
